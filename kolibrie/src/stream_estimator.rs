...
use crate::{container_stats::ContainerStats, streamertail_optimizer::{Condition, CostConstants, LogicalOperator}};
...
impl StreamEstimator {
    ...
    pub fn estimate_cost(&self, plan: &LogicalOperator) -> Option<i64> {
        match plan {
            // Scan
            ...
            LogicalOperator::Selection { predicate, condition } => {
                let input_cost = self.estimate_cost(predicate).unwrap();
            ...
            LogicalOperator::Projection { predicate, .. } => {
                Some(self.estimate_cost(predicate).unwrap() + CostConstants::COST_PER_PROJECTION as i64)
            }
            // Join
    ...
    fn extract_predicate_from_logical(&self, plan: &LogicalOperator) -> Option<u32> {
        match plan {
            LogicalOperator::Scan { pattern } => {
            ...
            LogicalOperator::Selection { predicate, ..  } => self.extract_predicate_from_logical(predicate),
            LogicalOperator::Projection { predicate, .. } => self.extract_predicate_from_logical(predicate),
            _ => None,
        }
    }
    ...
    fn compute_join_selectivity(&self, left: &LogicalOperator, right: &LogicalOperator) -> f64 {
    ...
    /// based on same method in estimator.rs
    pub fn estimate_output_cardinality(&self, plan: &LogicalOperator) -> i64 {
        match plan {
            LogicalOperator::Scan { pattern } => self.estimate_cardinality(pattern),
            LogicalOperator::Selection { predicate, condition } => {
                let input_cardinality = self.estimate_output_cardinality(predicate);
                let selectivity = self.estimate_selectivity(condition);
            ...
            LogicalOperator::Projection { predicate, .. } => self.estimate_output_cardinality(predicate),
            _ => 0
        }
    }
}